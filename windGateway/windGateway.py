...
class WindGateway(FhGateway):
    ...
    # ---------------------------------------
    def haveDaysData(self, code, start, end):
        data = self.w.wsd(code, 'close', start, end)
        # print data
        if data.ErrorCode == 0:
            return 1
        return 0
    ...
    # ---------------------------------------------
    def getDayData(self, code, start, end):
        d = {}
        data = self.w.wsd(code, 'close', start, end)
        if data.ErrorCode == 0:
            for i in range(0, len(data.Times)):
                if data.Data[0][i] == data.Data[0][i]:
                    d[data.Times[i]] = data.Data[0][i]
        a = sorted(d)
        retDict = OrderedDict()
        for key in a:
            retDict[key] = d[key]
    ...
    # ---------------------------------------------
    def haveTickData(self, code, start, end, barSize):
        legalList = [1, 3, 5, 10, 15, 30, 60]
        if barSize not in legalList:
            print u'barSiz参数不合法，请确认输入参数'
            return -1
        para = 'BarSize=' + str(barSize) + ';Fill=Previous;PriceAdj=F'
        # print 'wsi1:',datetime.datetime.now()
        data = self.w.wsi(code, 'close', start, end, para)
        # print 'wsi2:',datetime.datetime.now()
        startTime = '093001'
        endTime = '150001'
        if data.ErrorCode == 0:
            for i in range(0, len(data.Times)):
                if (data.Times[i].strftime('%H%M%S') > startTime) and (data.Times[i].strftime('%H%M%S') < endTime):
                    return 1
        return 0

    # -----------------------------------------
    def getTickData(self, code, start, end, barSize=15):
        legalList = [1, 3, 5, 10, 15, 30, 60]
        d = {}
        if barSize not in legalList:
            print u'barSiz参数不合法，请确认输入参数'
    ...
    # -----------------------------------------
    def getDayOpen(self, code):
        data = w.wsq(code, 'rt_open')
        if data.ErrorCode == 0:
            if data.Data[0][0] and data.Data[0][0] == data.Data[0][0]:
                return data.Data[0][0]
        return 0
