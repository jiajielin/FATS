...
# ======================================================
class CtpGateway(FhGateway):
    """CTP接口"""

    # ----------------------------------------------------------------------
    def __init__(self, eventEngine, gatewayName, dataEngine):
        """Constructor"""
        super(CtpGateway, self).__init__(eventEngine, gatewayName, dataEngine)
    ...
    def connect(self, cache={}):
        ...
        if cache:
            try:
                userID = str(cache['userID'])
                password = str(cache['password'])
                brokerID = str(cache['brokerID'])
                tdAddress = str(cache['tdAddress'])
                mdAddress = str(cache['mdAddress'])
            except KeyError:
                log = FhLogData()
    ...
    def query(self, event):
        ...
        # CTP有流控机制，所以查询不可太频繁，主要是针对交易服务器而言
    ...
    # ----------------------------------------------------------------------
    def immediateQueryPosition(self):
        self.tdApi.immediateFlag = True
        self.qryPosition()
...
class CtpMdApi(MdApi):
    ...
    def onRtnDepthMarketData(self, data):
        """行情推送，约0.5s推送一次"""
        # print 'CTP onRtnDepthMarketData:', datetime.datetime.now()
        # print 'data:',data
        tick = FhTickData()
        ...
        tick.fhSymbol = tick.symbol #'.'.join([tick.symbol, EXCHANGE_UNKNOWN])
...
class CtpTdApi(TdApi):
    ...
    def __init__(self, gateway):
        ...
        self.contractsPrexList = self.getContracts()
        print self.contractsPrexList
        self.contractsWidgetDict = {}
        self.immediateFlag = False          # 用于控制即时查询仓位标记
        
    # ----------------------------------------------------------------------
    def getContracts(self):
        retList = []
        try:
            with open('contractPrex.json') as f:
                contractsPrex = json.load(f)
                print contractsPrex
                for k in contractsPrex:
                    if contractsPrex[k]:
                        if k not in retList:
                            retList.append(k)
        except:
    ...
    def onRspQryInvestorPosition(self, data, error, n, last):
        ...
        if positionName in self.posBufferDict:
            ...
            if data['InstrumentID'] in self.contractsWidgetDict:
                posBuffer = PositionBuffer(data, self.gatewayName, self.contractsWidgetDict[data['InstrumentID']])
            else:
                posBuffer = PositionBuffer(data, self.gatewayName)
            self.posBufferDict[positionName] = posBuffer
        
        # 更新持仓缓存，并获取FH系统中持仓对象的返回值
        pos = posBuffer.updateBuffer(data)
        # print data
        # print 'onRspQryInvestorPosition', datetime.datetime.now()
        # 当为即时查询时
        if self.immediateFlag:
            # 改flag为非即时
            self.immediateFlag = False
            event1 = Event(type_=EVENT_POSITION)
            event1.dict_['data'] = pos
            # print 1
            self.gateway.dataEngine.updatePosition(event1)
        else:
    ...
    def onRspQryTradingAccount(self, data, error, n, last):
        """资金账户查询回报"""
        account = FhAccountData()
        account.gatewayName = self.gatewayName
        
        # 账户代码
        account.accountID = data['AccountID']
        account.fhAccountID = '.'.join([self.gatewayName, account.accountID])
    ...
    def onRspQryInstrument(self, data, error, n, last):
        ...
        # print data
        # print n, last
        contract = FhContractData()
        contract.gatewayName = self.gatewayName

        contract.symbol = data['InstrumentID']
        contract.exchange = exchangeMapReverse[data['ExchangeID']]
        contract.fhSymbol = contract.symbol #'.'.join([contract.symbol, contract.exchange])
        ...
        prex = ''
        for i in range(0, len(contract.symbol)):
            if contract.symbol[i] > '9' or contract.symbol[i] < '0':
                prex += contract.symbol[i]
            else:
                break
        if prex in self.contractsPrexList:
            self.contractsWidgetDict[contract.symbol] = contract.size
        ...
        if last:
            ...
            print self.contractsWidgetDict
            self.gateway.onLog(log)
    ...
    def onRtnOrder(self, data):
        ...
        order.fhSymbol = order.symbol #'.'.join([order.symbol, order.exchange])
        ...
        if data['CombOffsetFlag'] in offsetMapReverse:
            order.offset = offsetMapReverse[data['CombOffsetFlag']]
        else:
        ...
        # 考虑到FhTrader的应用场景，认为以上情况不会构成问题
        order.fhOrderID = '.'.join([self.gatewayName, order.orderID])
    ...
    def onRtnTrade(self, data):
        ...
        trade.fhSymbol = trade.symbol #'.'.join([trade.symbol, trade.exchange])
        
        trade.tradeID = data['TradeID']
        trade.fhTradeID = '.'.join([self.gatewayName, trade.tradeID])
        
        trade.orderID = data['OrderRef']
        trade.fhOrderID = '.'.join([self.gatewayName, trade.orderID])
    ...
    def qryPosition(self):
        ...
        req['BrokerID'] = self.brokerID
        req['InvestorID'] = self.userID
        # print 'qryPosition', datetime.datetime.now()
    ...
    def sendOrder(self, orderReq):
        ...
        fhOrderID = '.'.join([self.gatewayName, str(self.orderRef)])
        return fhOrderID
...
class PositionBuffer(object):
    ...
    处理成本价的问题，加入的costPrice.json的读取
    """

    # ----------------------------------------------------------------------
    def __init__(self, data, gatewayName, weight=0):
        """Constructor"""
        ...
        self.weight = weight
        self.price = EMPTY_FLOAT    # 昨日平均价
        
        # 通过提前创建持仓数据对象并重复使用的方式来降低开销
        pos = FhPositionData()
        pos.symbol = self.symbol
        pos.fhSymbol = self.symbol
        pos.gatewayName = gatewayName
        pos.direction = self.direction
        pos.fhPositionName = '.'.join([pos.fhSymbol, pos.direction])
        self.pos = pos
        self.getCostFlag = False
        self.getCostPrice()
        
    # ----------------------------------------------------------------------
    def getCostPrice(self):
        try:
            with open('costPrice.json') as f:
                costPirceDict = json.load(f)
                if self.symbol in costPirceDict:
                    if self.direction in costPirceDict[self.symbol]:
                        self.pos.price = costPirceDict[self.symbol][self.direction]['costPrice']
                        self.price = self.pos.price
                        self.getCostFlag = True
        except:
            self.pos.price = EMPTY_FLOAT
    ...
    def updateBuffer(self, data):
        ...
        self.openVolume = data['OpenVolume']
        self.closeVolume = data['CloseVolume']

        # 持仓的昨仓和今仓相加后为总持仓
        self.pos.position = self.openVolume - self.closeVolume + self.ydPosition
        self.pos.ydPosition = self.ydPosition
        self.pos.nowPrice = data['SettlementPrice']
        ...
        self.openAccount = data['OpenAmount']
        self.closeAccount = data['CloseAmount']

        oldPrice = self.pos.price
        # todo 需判断以下规则是否正确，主要是openAccount，closeAccount，todayPosition
        if self.getCostFlag:
            if self.weight and self.pos.position and self.price:
                self.pos.price = (self.price * self.weight * self.ydPosition + self.openAccount - self.closeAccount)/\
                                         (self.pos.position * self.weight)
            else:
                self.pos.price = 0
        else:
            if self.ydPosition:
                ...
                if self.weight:
                    if self.todayPosition:
                        self.pos.price = (self.openAccount - self.closeAccount) / (self.todayPosition * self.weight)
                    else:
                        self.pos.price = 0
                else:
                    self.pos.price = 0
        if self.pos.price != oldPrice:
            self.saveCostPrice()
            
        return copy(self.pos)

    # --------------------------------------------------------
    def saveCostPrice(self):
        now = datetime.datetime.now().strftime('%Y%m%d')
        costPrice = {}
        with open('costPriceToday.json') as f:
            try:
                costPrice = json.load(f)
                if now not in costPrice:
                    costPrice[now] = {}
                if self.pos.symbol not in costPrice[now]:
                    costPrice[now][self.pos.symbol] = {}
                costPrice[now][self.pos.symbol][self.pos.direction] = self.pos.price
            except:
                pass
        with open('costPriceToday.json', 'w') as f:
            if type(costPrice) == dict:
                f.write(json.dumps(costPrice, ensure_ascii=False, indent=2))
            else:
                f.write(json.dumps({}))
